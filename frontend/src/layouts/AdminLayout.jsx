...
import { LayoutDashboard, FileText, Users, Car, Wrench, Bell, Package, Settings, LogOut, Menu, X, MessageCircle, HelpCircle, User } from 'lucide-react'
...
const navItems = [
  ...
  { name: 'My Plan', href: '/admin/my-plan', icon: Package },
  { name: 'WhatsApp Settings', href: '/admin/whatsapp-settings', icon: MessageCircle },
  { name: 'Help & Support', href: '/admin/help-support', icon: HelpCircle },
...
export default function AdminLayout() {
  ...
  const NavSidebar = ({ onNavigate, slim = false, hideBrandTitle = false }) => (
    <div className="flex-1 flex flex-col pt-5 pb-4 overflow-y-auto">
      ...
      {!slim && user?.businessName && (
        <div className="px-4 mb-2">
          <p className="text-sm font-bold text-primary truncate bg-primary/10 px-2 py-1.5 rounded-lg">{user.businessName}</p>
        </div>
      )}
  ...
  return (
    <div className="flex h-screen bg-background">
      {/* Desktop: slim icon sidebar - light blue-grey bg, ~60px */}
      ...
      <div className="flex flex-col flex-1 md:pl-[60px]">
        {/* Header: no search, no notifications – profile + Sign Out only */}
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-3 min-h-[44px]">
              ...
              <div className="hidden sm:block min-w-0">
                {user?.businessName && (
                  <p className="text-sm font-bold text-primary truncate">{user.businessName}</p>
                )}
